...
- OCR + 図解抽出（Gemini 2.0 Flash Thinking）
...
- **Python 3.10+**
- **Google Gemini API**
  - `gemini-2.5-pro-latest` - OCR + 図解抽出（書字方向自動判定）
  - `gemini-2.5-flash-latest` - 翻訳（オプション）
...
#### 目標
- PDF → OCR → 日本語マークダウン生成
- 日本語マークダウン → 英語翻訳（Claude使用）
- シンプルなWebインターフェース

#### タスク
- [x] 要件定義・設計書作成
- [ ] Supabaseセットアップ（テーブル・Storage）
- [ ] バックエンド基盤構築（FastAPI）
- [ ] Gemini OCR実装
- [ ] Claude翻訳実装
- [ ] フロントエンド基本UI
