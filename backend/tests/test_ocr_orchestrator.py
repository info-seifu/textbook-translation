...
class TestOCROrchestrator:
    ...
    async def test_process_pdf_basic(
        ...
        """process_pdf - 基本処理フロー (PDF直接送信版)"""
        # モック設定 - PDF全体を送信して複数ページの結果を取得
        ocr_result_1 = sample_ocr_result
        ocr_result_2 = OCRResult(
            page_number=2,
            markdown_text="# ページ2\n\n2ページ目の内容。",
        ...
        mock_gemini_service.extract_from_pdf = AsyncMock(return_value=[ocr_result_1, ocr_result_2])
        ...
        mock_gemini_service.extract_from_pdf.assert_called_once_with("/tmp/test.pdf")
